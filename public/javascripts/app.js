...
function htmlEscape(str) {
  if (str.length != 1){
    ...
    // optimized for single character encoding.
    switch (str){
      case '&':  return '&amp;';
      case '"':  return '&quot;';
      case '\'': return "&#39;";
...
function format_text(input_text){
  ...
  for (var i = 0, l = input_text.length; i < l; i++){
    if (input_text[i] == '\b' && i-1 >= 0 && input_text[i-1] != "\n"){
      ...
      out_text.push(htmlEscape(input_text[i]));
    }
  }
...
$(function(){
  ...
  $('.heart_this').click(function(){
    ...
    $.post('/writing/'+writing._id+'/heart', function(resp){
      if (resp && resp.action){
        if (resp.action == 'hearted'){
          writing.hearts += 1;
          $('.unheart').removeClass('hide');
          $('.heart').addClass('hide');
        } else if (resp.action == 'unhearted'){
          writing.hearts -= 1;
          $('.heart').removeClass('hide');
          $('.unheart').addClass('hide')
        }
    ...
    if (type == 'email' || user_input.val()){
      $.post($(this).attr('submit_url'), {data: user_input.val()}, function(resp){
        if (resp && !resp.err){
          ...
          alert('error updating '+type+'. ' + (resp ? resp.err : ''));
        }
      });
    } else {
      alert('please type in a '+type+'.')
      user_input.focus();
    }
  });
